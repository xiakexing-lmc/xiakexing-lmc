...
# 输入: 形状为 (B, N, D)。
# 分离CLS Token: (B, 1, D) 和 (B, N-1, D)。
# 初始化选中/未选中Token: 根据初始化保持率进行分割。
# 扩展和合并: 更新选中Token的数量，合并未选中Token。
# 输出: 更新后的Token序列形状仍为 (B, N, D)，但包含了不同的Token集
class TokenSelect(nn.Module):
    ...
    # 此方法使用 bisect.bisect_right 来更新当前扩展阶段，基于给定的epoch和预定义的扩展步骤。
    ...
    # 根据定义的距离度量（余弦、曼哈顿、欧几里得），计算两组Token之间的距离或相似度分数。
    ...
    # 整合上述所有步骤，根据当前的训练阶段和策略进行Token的选择、扩展和合并
    def token_select(self, x):
        ...
        #包含了那些处理过程中被选中以保留的token索引
        # return x
        return x, select_index
    
    def forward(self,x):

        x, select_index = self.token_select(x)
        return x, select_index
