...
# 实现了窗口基多头自注意力机制（W-MSA）,使用了相对位置编码来增强模型对位置信息的理解
...
# 定义了Swin Transformer的基本块，包括一个归一化层、一个窗口注意力层和一个MLP层。这个结构支持层内信息的复杂变换和集成。
...
# 表示Swin Transformer中的一个阶段，由多个 SwinBlock 组成，并可能包括一个下采样层。这允许模型在不同尺度上处理信息，从而更好地捕获多尺度特征。
...
# 整合了多个 BasicLayer，构成了完整的Swin Transformer模型。这个类处理输入图像，通过一系列变换，最终输出用于后续任务的特征。
